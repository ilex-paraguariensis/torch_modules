...
class EncoderDecoderConvLSTM(nn.Module):
    def __init__(self, params, nf=76, in_chan=1):
    ...
    def forward(self, x, future_seq=6, hidden_state=None):
        future_seq = self.params["nc"]
...
class ConvLSTMTemporalDiscriminator(nn.Module):
    def __init__(self, params, nf=4, in_chan=1):
        super(ConvLSTMTemporalDiscriminator, self).__init__()

        self.params = params
        in_chan = params.n_channels

        self.use_cell_state_for_classification = True
        """ ARCHITECTURE 
        ...
        self.dense_layer = nn.Sequential(
            ...
            nn.Linear(
                params.imsize ** 2
                * nf
                * (2 if self.use_cell_state_for_classification else 1),
                1024,
            ),
            #  nn.ReLU(0.2, 0.3),
            nn.ReLU(),
        ...
        self.cell_state_dropout = nn.Dropout(0.4)

    def autoencoder(self, x, seq_len, future_step, h_t, c_t, h_t2, c_t2, h_t3, c_t3):
        ...
        return encoder_vector, cell_encoder
    ...
    def forward(self, x, future_seq=6, hidden_state=None):
        ...
        # autoencoder forward
        h, c = self.autoencoder(
            x, seq_len, future_seq, h_t, c_t, h_t2, c_t2, h_t3, c_t3
        )
        # outputs = outputs.permute(0, 2, 1, 3, 4).squeeze(2)
        # ipdb.set_trace()
        if self.use_cell_state_for_classification:
            c = self.cell_state_dropout(c)
            x = torch.cat((h, c), 1)
        else:
            x = h
        # ipdb.set_trace()
